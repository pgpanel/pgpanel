...
use pgpanel_databasus::{DatabasusAdapter, RegisterRequest};
...
impl JobContext {
    pub async fn handle(&self, op: Operation) -> Result<serde_json::Value> {
        match op.job_type {
            ...
            JobType::RegisterDatabasus => self.handle_register_databasus(&op).await,
            ...
            JobType::RefreshMetrics => Ok(serde_json::json!({"ok": true})),
        }
    }
    ...
    async fn handle_create_cluster(&self, op: &Operation) -> Result<serde_json::Value> {
        ...
        // Databasus registration (failure must not kill the cluster)
        if req.enable_backup {
            self.queue
                .set_progress(op.id, 85, "registering with Databasus")
                .await?;
            if let Err(e) = self
                .register_databasus_inner(cluster_id, &names.internal_hostname)
                .await
            {
                self.queue
                    .append_log(op.id, "warn", &format!("Databasus: {e}"))
                    .await?;
                self.set_cluster_status(
                    cluster_id,
                    ClusterStatus::HealthyWithBackupWarning,
                    Some(&format!("backup registration failed: {e}")),
                )
                .await?;
    ...
    async fn handle_register_databasus(&self, op: &Operation) -> Result<serde_json::Value> {
        let cluster_id = op
            .cluster_id
            .ok_or_else(|| Error::Job("missing cluster_id".into()))?;
        let host: String =
            sqlx::query_scalar("SELECT internal_hostname FROM clusters WHERE id = ?")
                ...
                .await
                .map_err(|e| Error::Job(e.to_string()))?;
        self.register_databasus_inner(cluster_id, &host).await?;
        // If previously HealthyWithBackupWarning, upgrade
        self.set_cluster_status(cluster_id, ClusterStatus::Healthy, None)
            .await?;
        Ok(serde_json::json!({"registered": true}))
    }

    async fn register_databasus_inner(&self, cluster_id: Uuid, host: &str) -> Result<()> {
        let (name, version, port): (String, String, Option<i64>) =
            sqlx::query_as("SELECT name, postgres_version, public_port FROM clusters WHERE id = ?")
                .bind(cluster_id.to_string())
                .fetch_one(&self.pool)
                .await
                .map_err(|e| Error::Job(e.to_string()))?;

        // Prefer backup role credentials if present
        ...
        let reg = self.databasus.register_cluster(req).await?;
        ...
        sqlx::query(
            ...
            ON CONFLICT(cluster_id) DO UPDATE SET
                ...
                manual_setup_info = excluded.manual_setup_info,
        ...
        .bind(&reg.external_id)
        .bind(&reg.message)
        .bind(reg.manual_setup_info.as_ref().map(|v| v.to_string()))
        .bind(&now)
        .bind(&now)
        .execute(&self.pool)
        .await
        ...
        sqlx::query("UPDATE clusters SET databasus_status = ?, updated_at = ? WHERE id = ?")
            .bind(reg.status.as_str())
            .bind(&now)
            .bind(cluster_id.to_string())
            .execute(&self.pool)
        ...
        if matches!(reg.status, DatabasusIntegrationStatus::Error) {
            return Err(Error::Databasus(
                reg.message.unwrap_or_else(|| "registration failed".into()),
            ));
        }
        Ok(())
    }
