...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/api/fleet/remote-access", get(remote_access).put(set_remote_access))
        .route("/api/fleet/remote-access/rotate-token", post(rotate_token))
        .route("/api/fleet/invites", get(list_invites).post(create_invite))
        .route("/api/fleet/invites/{id}", axum::routing::delete(revoke_invite))
...
fn prefix(s: &str) -> String { s.chars().take(12).collect() }
fn now() -> String { Utc::now().to_rfc3339() }

#[derive(serde::Deserialize)] struct RemoteBody { enabled: bool, #[serde(default)] public_base_url: String }
#[derive(serde::Serialize, sqlx::FromRow)] struct RemoteRow {
    enabled: i64, public_base_url: String, agent_token_prefix: Option<String>, updated_at: String,
}
...
async fn remote_access(State(s): State<AppState>, _a: AuthUser) -> ApiResult<Json<serde_json::Value>> {
...
async fn rotate_token(State(s): State<AppState>, a: AuthUser) -> ApiResult<Json<serde_json::Value>> {
    require_admin(&a)?;
    let t = token();
    let base: String = sqlx::query_scalar("SELECT public_base_url FROM panel_remote_access WHERE id=1")
        .fetch_one(&s.pool)
        .await
        .map_err(|e| AppError(Error::Internal(e.to_string())))?;
    sqlx::query(
...
#[derive(serde::Deserialize)] struct InviteBody { #[serde(default)] label: String, expires_in_hours: Option<i64> }
async fn create_invite(State(s): State<AppState>, a: AuthUser, Json(b): Json<InviteBody>) -> ApiResult<Json<serde_json::Value>> {
    require_admin(&a)?; let t = token(); let id = Uuid::new_v4().to_string(); let expires = b.expires_in_hours.map(|h| (Utc::now()+Duration::hours(h.clamp(1, 720))).to_rfc3339());
    let base: String = sqlx::query_scalar("SELECT public_base_url FROM panel_remote_access WHERE id=1").fetch_one(&s.pool).await.map_err(|e| AppError(Error::Internal(e.to_string())))?;
    if base.trim().is_empty() { return Err(AppError(Error::Validation("public_base_url is required".into()))); }
...
async fn list_invites(State(s): State<AppState>, _a: AuthUser) -> ApiResult<Json<Vec<serde_json::Value>>> {
    #[derive(sqlx::FromRow)]
    struct InviteRow {
        id: String,
        token_prefix: String,
...
async fn revoke_invite(State(s): State<AppState>, a: AuthUser, Path(id): Path<String>) -> ApiResult<Json<serde_json::Value>> {
    require_admin(&a)?; sqlx::query("UPDATE fleet_invite_tokens SET revoked_at=? WHERE id=?").bind(now()).bind(id).execute(&s.pool).await.map_err(|e| AppError(Error::Internal(e.to_string())))?; Ok(Json(serde_json::json!({"ok":true})))
}

async fn bearer(headers: &HeaderMap, s: &AppState) -> Result<String, AppError> {
    let raw = headers.get(header::AUTHORIZATION).and_then(|v|v.to_str().ok()).and_then(|v| v.strip_prefix("Bearer ")).ok_or(AppError(Error::Unauthorized))?;
    let h = hash(raw);
    ...
    if ok.is_none() { return Err(AppError(Error::Unauthorized)); } Ok(raw.to_string())
}
async fn accept_invite(State(s): State<AppState>, headers: HeaderMap) -> ApiResult<Json<serde_json::Value>> {
    let raw = headers.get(header::AUTHORIZATION).and_then(|v|v.to_str().ok()).and_then(|v|v.strip_prefix("Bearer ")).ok_or(AppError(Error::Unauthorized))?;
    ...
    if exp.as_deref().and_then(|x| chrono::DateTime::parse_from_rfc3339(x).ok()).map(|x| x < Utc::now()).unwrap_or(false) { return Err(AppError(Error::Unauthorized)); }
    sqlx::query("UPDATE fleet_invite_tokens SET used_at=? WHERE id=?").bind(now()).bind(id).execute(&s.pool).await.ok();
...
async fn agent_info_inner(s: &AppState) -> Result<serde_json::Value, AppError> {
    let (count,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM clusters").fetch_one(&s.pool).await.map_err(|e| AppError(Error::Internal(e.to_string())))?;
    let max: Option<i64> = sqlx::query_scalar("SELECT max_clusters FROM nodes WHERE is_default=1 LIMIT 1").fetch_optional(&s.pool).await.map_err(|e| AppError(Error::Internal(e.to_string())))?;
    let name: String = sqlx::query_scalar("SELECT value FROM settings WHERE key='fleet.public_name'").fetch_optional(&s.pool).await.map_err(|e| AppError(Error::Internal(e.to_string())))?.unwrap_or_else(|| "PgPanel node".into());
    Ok(serde_json::json!({"name":name,"version":env!("CARGO_PKG_VERSION"),"cluster_count":count,"max_clusters":max,"status":"online"}))
}
async fn agent_info(State(s): State<AppState>, h: HeaderMap) -> ApiResult<Json<serde_json::Value>> { bearer(&h,&s).await?; Ok(Json(agent_info_inner(&s).await?)) }

#[derive(serde::Deserialize)] struct JoinBody { base_url: String, token: String, local_name: Option<String> }
async fn join(
    State(s): State<AppState>,
    a: AuthUser,
    ...
    let cluster_count = info.get("cluster_count").and_then(|x| x.as_i64()).unwrap_or(0);
...
async fn remove_peer(State(s): State<AppState>, a: AuthUser, Path(id): Path<String>) -> ApiResult<Json<serde_json::Value>> { require_admin(&a)?; sqlx::query("DELETE FROM fleet_peers WHERE id=?").bind(id).execute(&s.pool).await.map_err(|e|AppError(Error::Internal(e.to_string())))?; Ok(Json(serde_json::json!({"ok":true})))}
async fn ping_peer(State(s): State<AppState>, a: AuthUser, Path(id): Path<String>) -> ApiResult<Json<serde_json::Value>> { require_admin(&a)?; let row: (String,String)=sqlx::query_as("SELECT base_url,token_encrypted FROM fleet_peers WHERE id=?").bind(&id).fetch_one(&s.pool).await.map_err(|_e|AppError(Error::NotFound("peer".into())))?; let t=pgpanel_core::crypto::decrypt_secret(&s.config.master_encryption_key,&row.1).map_err(AppError)?.expose_secret().to_string(); let r=reqwest::Client::new().get(format!("{}/api/fleet/agent/info",row.0.trim_end_matches('/'))).bearer_auth(t).send().await; match r { Ok(resp) if resp.status().is_success()=>{let v=resp.json::<serde_json::Value>().await.unwrap_or_default(); sqlx::query("UPDATE fleet_peers SET status='online',cluster_count=?,advertised_capacity=?,last_seen_at=?,last_error=NULL,updated_at=? WHERE id=?").bind(v["cluster_count"].as_i64().unwrap_or(0)).bind(v["max_clusters"].as_i64()).bind(now()).bind(now()).bind(id).execute(&s.pool).await.ok(); Ok(Json(serde_json::json!({"status":"online","info":v})))}, Ok(resp)=>Ok(Json(serde_json::json!({"status":"offline","error":resp.status().to_string()}))), Err(e)=>Ok(Json(serde_json::json!({"status":"offline","error":e.to_string()}))) } }
