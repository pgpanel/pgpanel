...
generate_or_load_secrets() {
  ...
  if [[ -f "$env_file" ]]; then
    ...
    # Networking / Databasus defaults for upgrades from older installers
    upsert_env_key "$env_file" "PGPANEL_MANAGEMENT_NETWORK" "pgpanel_database_management"
    if ! grep -q '^DATABASUS_HTTP_API=' "$env_file" 2>/dev/null; then
      upsert_env_key "$env_file" "DATABASUS_HTTP_API" "0"
    fi
    upsert_env_key "$env_file" "DATABASUS_BASE_URL" "http://databasus:8000"
    return 0
  fi
...
# ── Databasus ───────────────────────────────────────────────────────────────
ENABLE_DATABASUS=${ENABLE_DATABASUS}
# Always internal Docker DNS — never a public hostname for panel→Databasus calls
...
DATABASUS_DOMAIN=${DATABASUS_DOMAIN}
DATABASUS_PUBLIC=${DATABASUS_PUBLIC}
# 0 = Manual adapter (recommended). Invented /api/v1/* paths cause hard failures.
DATABASUS_HTTP_API=0
...
render_compose() {
  ...
  # Databasus is always on internal + management (reach PG clusters by DNS).
  # Frontend network only when explicitly public.
...
#
# Networks:
#   pgpanel_frontend            — Caddy ↔ panel (public edge)
#   pgpanel_internal            — panel ↔ Databasus (stack services)
...
services:
  ...
  panel:
    # Pre-built image only — never built on the VPS by the installer
    ...
    environment:
      ...
      DATABASUS_HTTP_API: \${DATABASUS_HTTP_API:-0}
    volumes:
...
build_and_start() {
  ...
  (
    ...
    docker compose -f compose.yml pull caddy databasus \
      || die "Failed to pull Caddy/Databasus images (check network)"
...
${C_YELLOW}FIGYELEM:${C_RESET}
  ...
  • PostgreSQL clusterek csak a pgpanel_database_management hálózaton
    érhetők el (pgpanel_pg_<slug>); nincs publikus PG domain.
  • Databasus regisztráció: manuális a UI-ban (DATABASUS_HTTP_API=0).
...
prompt_network() {
  ...
  echo "${C_DIM}PostgreSQL clusterek soha nincsenek publikus domainen."
  echo "Databasus alapból csak a Docker hálózaton él (http://databasus:8000).${C_RESET}"
  ...
  if [[ "$USE_DOMAIN" -eq 1 ]]; then
    ...
    # Databasus public domain is optional and OFF by default
    prompt_yesno DATABASUS_PUBLIC "Databasus is publikus aldomainen? (ajánlott: nem)" "n"
    if [[ "$DATABASUS_PUBLIC" -eq 1 ]]; then
      local default_db_domain=""
      if [[ -n "$PANEL_DOMAIN" && "$PANEL_DOMAIN" != *example.com ]]; then
        default_db_domain="backup.${PANEL_DOMAIN#*.}"
        # e.g. panel.example.com → backup.example.com; if single label keep backup.<panel>
        if [[ "$PANEL_DOMAIN" != *.* ]]; then
          default_db_domain="backup.${PANEL_DOMAIN}"
        elif [[ "$(echo "$PANEL_DOMAIN" | tr '.' '\n' | wc -l | tr -d ' ')" -eq 2 ]]; then
          default_db_domain="backup.${PANEL_DOMAIN}"
        else
          # strip first label: app.db.example.com → backup.db.example.com
          default_db_domain="backup.${PANEL_DOMAIN#*.}"
        fi
      fi
      prompt_val DATABASUS_DOMAIN "Databasus domain" "${DATABASUS_DOMAIN:-$default_db_domain}"
      [[ -n "$DATABASUS_DOMAIN" ]] || die "DATABASUS_DOMAIN required when public"
    else
      DATABASUS_DOMAIN=""
      log_info "Databasus: internal only (http://databasus:8000 on Docker network)"
    fi
  else
    ENABLE_HTTPS=0
    DATABASUS_PUBLIC=0
  ...
  prompt_yesno PUBLIC_PG_PORTS "Publikus PostgreSQL host-portok engedélyezése alapból? (NEM ajánlott)" "n"
  if [[ "$PUBLIC_PG_PORTS" -eq 1 ]]; then
    log_warn "Publikus PG portok: clusterek expose_publicly=true esetén host-ra mapelődnek. Preferáld VPN/tunnel-t."
  fi
...
show_summary_and_confirm() {
  local databasus_disp
  if [[ "$DATABASUS_PUBLIC" -eq 1 && -n "$DATABASUS_DOMAIN" ]]; then
    databasus_disp="public https://${DATABASUS_DOMAIN}"
  else
    databasus_disp="internal only (http://databasus:8000)"
  fi
  local pg_ports_disp="private (Docker network only)"
  [[ "$PUBLIC_PG_PORTS" -eq 1 ]] && pg_ports_disp="public host ports allowed"
  cat <<EOF

${C_BOLD}Telepítési összegzés${C_RESET}

  Panel domain:        ${PANEL_DOMAIN:-"(HTTP-only / no domain)"}
  Databasus:           ${databasus_disp}
  PostgreSQL:          ${pg_ports_disp}
...
do_repair() {
  ...
  upsert_env_key "${INSTALL_DIR}/.env" "DATABASUS_HTTP_API" "0"
  upsert_env_key "${INSTALL_DIR}/.env" "DATABASUS_BASE_URL" "http://databasus:8000"
  if [[ "${DATABASUS_PUBLIC:-0}" -ne 1 ]]; then
    upsert_env_key "${INSTALL_DIR}/.env" "DATABASUS_PUBLIC" "0"
    # Do not invent a public domain for private Databasus
    if grep -qE '^DATABASUS_DOMAIN=backup\.db\.example\.com$' "${INSTALL_DIR}/.env" 2>/dev/null; then
      upsert_env_key "${INSTALL_DIR}/.env" "DATABASUS_DOMAIN" ""
    fi
  fi
