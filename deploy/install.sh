...
ensure_databasus_data_dir() {
  ...
  chmod 0755 "$root"
  # A previous permission failure can leave an empty/incomplete pgdata tree that
  # blocks initdb once the parent directory becomes traversable.
  if [[ -d "$pgdata" ]] && [[ ! -f "${pgdata}/PG_VERSION" ]]; then
    rm -rf "$pgdata"
  fi
}

reset_databasus_container() {
  [[ "$ENABLE_DATABASUS" -eq 1 ]] || return 0
  [[ -f "${INSTALL_DIR}/deploy/compose.yml" ]] || return 0
  (
    cd "${INSTALL_DIR}/deploy"
    docker compose stop databasus >/dev/null 2>&1 || true
    docker compose rm -f databasus >/dev/null 2>&1 || true
  ) || true
}

# Bring the whole stack up without letting a recovering Databasus crash-loop
# block Caddy/panel. Databasus is prepared and started first when enabled.
compose_up_stack() {
  local no_build_flag="${1:---no-build}"
  local pull_policy="${PGPANEL_PULL_POLICY:-missing}"
  ensure_databasus_data_dir
  reset_databasus_container
  ...
  (
    ...
    set +a
    export PGPANEL_PULL_POLICY="$pull_policy"

    if [[ "$ENABLE_DATABASUS" -eq 1 ]]; then
      # shellcheck disable=SC2086
      docker compose up -d --no-deps ${no_build_flag} databasus
      if ! wait_for_container_health databasus 90; then
        log_warn "Databasus not healthy yet — repairing data dir and recreating"
        ensure_databasus_data_dir
        # shellcheck disable=SC2086
        docker compose up -d --no-deps --force-recreate ${no_build_flag} databasus
        wait_for_container_health databasus 90 \
          || die "Databasus failed to become healthy after permission repair"
      fi
      log_ok "Databasus ready"
    fi

    # shellcheck disable=SC2086
    docker compose up -d --remove-orphans ${no_build_flag}
  )
}
...
render_compose() {
  ...
  # Do NOT gate Caddy on Databasus health. A recovering sidecar must never
  # block the panel proxy / Cloudflare Tunnel origin.
...
build_and_start() {
  ...
  # install or resume.
  if ! PGPANEL_PULL_POLICY=never compose_up_stack "--no-build"; then
    ...
    prepare_compose_networks
    ensure_panel_image
    PGPANEL_PULL_POLICY=never compose_up_stack "--no-build"
  fi
...
do_update() {
  ...
  if [[ "$update_host" -eq 1 ]]; then
    ...
    ) || die "Host service image pull failed after panel cutover"
    compose_up_stack "--no-build" || die "Host service update failed after panel cutover"
  fi
...
do_repair() {
  ...
  if have_cmd docker; then
    compose_up_stack "--no-build" || log_error "compose up failed"
